        ...
        <div class="col-lg-12">
            <x-card>
                <x-slot name="header">
                    ...
                    @php
                        $statusTerkirim = \App\Models\MerdekaNilaiFormatif::where('rombel_id', $rombel->id)
                    ...
                    <div class="btn-group text-center text-md-right float-right">
                        @if (!$statusTerkirim)
                            <a href="{{ route('nilaiformatif.create', [$rombel->id, $mataPelajaran->id]) }}"
                ...
                @php
                    ...
                    $nilaiSUM = \App\Models\MerdekaNilaiFormatif::where('rombel_id', $rombel->id)
                        ->where('mata_pelajaran_id', $mataPelajaran->id)
                        ->orderBy('sum')
                        ->get()
                        ->groupBy('sum');
                    ...
                    <x-table id="nilai-harian-table" data-rombel-id="{{ $rombel->id }}"
                        ...
                        <x-slot name="thead">
                            <tr>
                                ...
                                @if ($nilaiSUM->isNotEmpty())
                                    @foreach ($nilaiSUM as $sum => $nilai)
                                        <th class="text-center">
                                            <div class="d-flex justify-content-between align-items-center flex-wrap">
                                                <span>Bab {{ $sum }}</span>
                                                @if (!$statusTerkirim)
                                                    <div class="btn-group">
                                                        <a href="{{ route('nilaiformatif.edit', [$rombel->id, $mataPelajaran->id, $sum]) }}"
        ...
        function kirimNilai() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('nilaiformatif.kirim', [$rombel->id, $mataPelajaran->id]) }}",
                        type: "POST",
                        headers: {
                            "X-CSRF-TOKEN": "{{ csrf_token() }}"
                        },
        ...
        function batalKirim() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('nilaiformatif.batalKirim', [$rombel->id, $mataPelajaran->id]) }}",
                        type: "POST",
                        headers: {
                            "X-CSRF-TOKEN": "{{ csrf_token() }}"
                        },
